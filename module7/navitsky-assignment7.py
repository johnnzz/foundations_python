...
Create a binary file called todo.p that contains the data.

When the program starts, load the each row of data you have in the todo.p
...
Save the data from the table into the todo.p file when the program exits.

Additional Assignment 7 Instructions:

    Demonstrate error handling.

    Demonstrate use of pickling/binary file format.
'''
...
class todo():
    ...
    def save_data(_task_list = [], _file_name = "todo.p"):
        """
        ...
        try:
            ...
            _todo_file = open(_file_name, "wb")
            pickle.dump(_task_list, _todo_file)
            _todo_file.close()

            _msg="Successfully wrote tasks to " + _file_name + "!"

        except Exception as _py_err:
            # we don't have specific error handling here because we've already
            # checked for file existence, write access, previously
            _error=True
    ...
    def read_data(_file_name = "todo.p"):
        """
        read the existing records from the file.  if the file doesn't exist
        or is null, attempt to write an empty table to the file to catch
        serious problems before the user enters data.
        ...
        try:
            ...
            _todo_file = open(_file_name, "rb")
            _task_list = pickle.load(_todo_file)
            _todo_file.close()

            # figure out what the last id is so we can compute an id for new record

            # scan the list for the largest id.
            for _record in _task_list:
                if int(_record["id"]) > _last_id:
                    _last_id = int(_record["id"])

        except ( EOFError, FileNotFoundError ) as _py_err:
            # we couldn't read from file because it's empty or doesn't
            # exist.
            try:
                # try to write an empty list to the file to verify we can.
                # if we can, no harm, no foul.
                _error, _msg = todo.save_data(_task_list, _file_name)
            except Exception as _py_err:
                # we couldn't write to the empty file, maybe a permissions issue?
                _error=True
                _error_msg="Couldn't write to " + _file_name + ", check permissions.\n" + str(_py_err)

        except Exception as _py_err:
            # unknown error, possibly file corruption/unexpected file contents
            _error=True
...
# save a "before" version of the file which both insures we can write
# to the directory as well as provides a backup in case something goes
# wrong.  then we save to the "normal" file just to verify we can write
# to it -- if we're going to fail, fail before the user starts entering
# data.
for file_extension in [ ".save", "" ]:
    error, msg = todo.save_data(task_list, file_name + file_extension )
    if error:
        # can't write to file so we can't proceed
        todo.croak(msg)
...
while(True):
    ...
    if command in [ "2", "r", "remove", "d", "del", "delete", "e", "erase" ]:
        print()
